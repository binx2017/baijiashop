...
class GoodsAttributeController extends Controller{
    ...
    public function setAction(){
        if (IS_POST){
        $goods_attribute_id = I('post.goods_attribute_id','');
        $model = D('GoodsAttribute');
            if ($model->create()){
                ...
                $param =[];
                if ($goods_attribute_id !== ''){
                    $param['goods_attribute_id'] = $goods_attribute_id;
                }
                $this->redirect('set',$param);
            }
        }else{
            $goods_attribute_id = I('get.goods_attribute_id','');
            ...
            if(!$data && $goods_attribute_id){
                $data = M('GoodsAttribute')->find($goods_attribute_id);
            }
