...
class StockStatusController extends Controller{
    ...
    public function setAction(){
        if (IS_POST){
        $stock_status_id = I('post.stock_status_id','');
        $model = D('StockStatus');
            if ($model->create()){
                if ($stock_status_id === ''){
                    $model->add();
                ...
                $param =[];
                if ($stock_status_id !== ''){
                    $param['stock_status_id'] = $stock_status_id;
                }
                $this->redirect('set',$param);
            }
        }else{
            $stock_status_id = I('get.stock_status_id','');
            ...
            if(!$data && $stock_status_id){
                $data = M('StockStatus')->find($stock_status_id);
            }
